...
load_dyn interp hsc_env crash_early dll
  ...
  -- On NixOS, trying to load these via dlopen can load a different version than what
...
-- copy (which can happen on NixOS where gcc might find a different version).
