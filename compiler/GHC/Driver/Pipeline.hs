        ...
        case ghcLink dflags of
          LinkExecutable blm ->
            ...
            in if | backendUseJSLinker (backend dflags) ->
                  ...
                  | otherwise ->
                      linkExecutableLinkable $ \obj_files -> do
          ...
          LinkStaticLib ->
            linkObjectLinkable $ \obj_files -> do
              apply_objs <- autoApplyObjsForLink hsc_env pkg_deps
  ...
  where
    ...
    variant_flags rts_incs mb_vec d0 =
      let ts = toolSettings d0
          ...
          d3 | x86 = d2 { sseAvxVersion = Nothing, bmiVersion = Nothing
                        , avx512cd = False, avx512er = False
                        , avx512f = False, avx512pf = False
                        , fma = False }
             | otherwise = d2
      in case mb_vec of
           Just 32 | x86 -> d3 { sseAvxVersion = Just AVX2 }
