...
module GHC.StgToCmm.AutoApply
  ( genAutoApply
    -- * The Cmm files GHC generates and links along with the RTS
  , GenFile(..)
  , genFiles
  , genFileVecWidth
  , genFile
...
tagBitsMax platform = mAX_PTR_TAG platform + 1
...
Note [Linking the generic apply code] in GHC.Driver.Pipeline.  'genFiles'
lists them, 'genFile' produces each one's source, and 'genFileVecWidth' says
which vector width (and hence which -mavx flag) a file needs.  Jumps.h itself
stays in the RTS as an installed header.
-}
