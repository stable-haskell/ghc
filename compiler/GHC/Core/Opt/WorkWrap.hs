  ...
  = warnPprTrace (not (wrap_dmds `lengthIs` (arityInfo fn_info)))
    ...
    do { mb_stuff <- mkWwBodies ww_opts fn_id arg_vars (exprType body) wrap_dmds cpr
       ; case mb_stuff of
            ...
            Just stuff
              | let opt_wwd_rhs = mkLams arg_vars (simpleOptExpr (wo_simple_opts ww_opts) body)
                  -- Run the simple optimiser on the WW'd bod, to get rid of junk
                  -- Keep all the binders though -- this might be a join point
