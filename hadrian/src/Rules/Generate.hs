...
generatePackageCode context@(Context stage pkg _ _) = do
    ...
    when (pkg == rts) $ do
        -- The generic apply code is generated by the same compiler that
        -- compiles this stage's RTS, with @ghc --gen-apply@; the vector
        -- variants live in separate files, see Note [AutoApply.cmm for
        -- vectors] in GHC.StgToCmm.AutoApply.
        forM_ [ ("AutoApply.cmm",     Nothing)
              , ("AutoApply_V16.cmm", Just 16)
              , ("AutoApply_V32.cmm", Just 32)
              , ("AutoApply_V64.cmm", Just 64) ] $ \(cmm, mb_vec) ->
          root -/- "**" -/- dir -/- "cmm" -/- cmm %> \file -> do
            -- GHC reads the target's platform constants from this stage's
            -- DerivedConstants.h; see Note [Target information for the
            -- generic apply code] in GHC.StgToCmm.AutoApply.
            path <- buildPath context
            let h = path -/- "include/DerivedConstants.h"
            need [h]
            build $ target context (Ghc (GenApply mb_vec) stage) [h] [file]
