...
# Generate config.status without creating outputs
config.status: configure
	@./configure --no-create

# Files that will be generated by config.status from their .in counterparts
# Note: keep this in sync with configure.ac
CONFIGURED_FILES := \
	compiler/ghc.cabal \
	ghc/ghc-bin.cabal \
...
# Regenerate a configured file when its .in or config.status changes
...
$(BUILD_DIR)/booted: $(CONFIGURE_SCRIPTS) $(CONFIGURED_FILES)
	$(call GROUP,Configured sources are up-to-date)
	@mkdir -p $(@D)
	@touch $@
	$(END_GROUP)
...
.PHONY: clean clean-everything clean-stage1 clean-stage2 distclean test all
