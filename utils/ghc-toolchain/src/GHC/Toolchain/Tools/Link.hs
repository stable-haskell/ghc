...
linkSupportsVerbatimNamespace cc ar ranlib ccLink = checking "whether cc linker supports -l:libfoo.a" $ withTempDir $ \tmpDir -> do
    ...
    code <- runProgram ccLink [main_c, "-o", main', "-L" ++ tmpDir, "-l:libtest.a"]
    exists <- liftIO $ doesFileExist main'
    case code of
      ExitSuccess
        | exists -> return True
      _ -> return False
